...
def detect_video(yolo, video_path, output_path=""):
    ...
    while True:
        if zed.grab(runtime_params)==tp.PyERROR_CODE.PySUCCESS:
            ...
            frame0=frame0[round(0.25*width):round(0.75*width),round(height/2):height]
            ...
            global locX,locY,num_cone
            #err,depth_value=depth.get_value(locX,locY)
            err,point_cloud_value=point_cloud.get_value(locX,locY)
            distance=math.sqrt(point_cloud_value[0]*point_cloud_value[0]+point_cloud_value[1]*point_cloud_value[1]+point_cloud_value[2]*point_cloud_value[2])
            
            frame_width,frame_height=frame.size
            camera_x=float(point_cloud_value[2]*locX-point_cloud_value[2]*frame_width/2)/349.614
            camera_y=float(point_cloud_value[2]*locY-point_cloud_value[2]*frame_height/2)/349.614
            camera_z=point_cloud_value[2]
            #distance=camera_x*camera_x+camera_y*camera_y+camera_z*camera_z
            msg=io.StringIO()
            """global Class
            if not np.isnan(depth_value) and not np.isinf(distance):
                print("camera_x{0}camera_y{1}camera_z{2}\n".format(camera_x,camera_y,camera_z))
                print("distance to the {0} at ({1},{2}) is: {3}mm\n".format(class_list[Class],locX,locY,distance))"""
            
            class_list=["blue","red","yellow"]
            if not np.isnan(distance) and not np.isinf(distance):
                distance=round(distance)
                global Class
                print("Distance to %s at(%d，%d):%d mm"%(class_list[Class],locX,locY,distance))
                #msg="{%s}{%d}x{%d}y{%d}z{%d}dis{%d}".format(class_list[Class],num_cone,camera_x,camera_y,camera_z,distance)
                msg="{0}{1}x{2}y{3}z{4}dis{5}\n".format(class_list[Class],num_cone,camera_x,camera_y,camera_z,distance)
                print(msg)
                ...
                msg="{0}{1}x{2}y{3}z{4}".format(class_list[Class],num_cone,camera_x,camera_y,camera_z)
                print(msg)
                client.sendto(msg.encode('utf-8'),ip_port)
                data,server_addr=client.recvfrom(BUFSIZE)
                #Sleep(10)
            ...
            """if not np.isnan(distance) and not np.isinf(distance):
                ...
                global Class
                if Class==0:
                    sprintf(msg,"{}{}x{}y{}z{}dis{}"%(class_list[Class],num_cone,camera_x,camera_y,camera_z,distance))
                    print("Distance to Blue Cone at(%d，%d):%d mm\n"%(locX,locY,distance))
                    print("camera_x%dcamera_y%dcamera_z%d"%(camera_x,camera_y,camera_z))
                elif Class==1:
                    #sprintf(msg,"Distance to Red Cone at(%d, %d):%d mm\n"%(locX,locY,depth_value))
                    print("Distance to red Cone at(%d，%d):%d mm\n"%(locX,locY,distance))
                    print("camera_x%dcamera_y%dcamera_z%d"%(camera_x,camera_y,camera_z))
                else:
                    #sprintf(msg,"Distance to Yellow Cone at(%d, %d):%d mm\n"%(locX,locY,depth_value))
                    print("Distance to yellow Cone at(%d，%d):%d mm\n"%(locX,locY,distance))
                    print("camera_x%dcamera_y%dcamera_z%d"%(camera_x,camera_y,camera_z))
